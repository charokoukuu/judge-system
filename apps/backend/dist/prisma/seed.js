...
async function main() {
    ...
    await prisma.verdict.create({
        data: {
            ...
            rationale: "右側の論者は、具体的な解決策と長期的視点を提示し、反論に対しても建設的な回答を行いました。データ駆動な議論展開が評価されます。",
        },
    });
    const session2 = await prisma.session.create({
        data: {
            id: "session_2",
            theme: "AIは人間の雇用を奪うか",
            state: client_1.SessionState.TURN1_RIGHT,
    ...
    const session3 = await prisma.session.create({
        data: {
            ...
            state: client_1.SessionState.READY,
    ...
    await prisma.verdict.create({
        data: {
            sessionId: session4.id,
            winner: client_1.Winner.RIGHT,
            rationale: "右側は将来性と教育効果を軸に一貫した主張を展開。左側の懸念点にも具体的な解決策を提示し、説得力のある議論でした。",
        },
    });

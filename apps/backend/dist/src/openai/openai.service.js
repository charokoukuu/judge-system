...
let OpenaiService = OpenaiService_1 = class OpenaiService {
    ...
    async chatCompletion(messages, model = "gpt-4o-mini") {
        this.logger.log(`Requesting chat completion with ${messages.length} messages`);
        try {
            ...
            this.logger.error("Error during chat completion:", error.response?.data || error.message);
